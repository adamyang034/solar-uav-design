...
Usage:  .venv/bin/python pi_tail_maxeon_gen5/scripts/run_phase34.py
"""
...
def test_site_motor_boom_fuse_pack():
    ...
    check("chord floor is 300 mm, start 340 mm",
          abs(config.CHORD_BOUNDS_M[0] - 0.300) < 1e-9
          and abs(config.CHORD_BOUNDS_M[1] - 0.520) < 1e-9
          and abs(config.OPT_START["chord_m"] - 0.340) < 1e-9)
    ...
    from solar_uav.aircraft import (reference_design, fuselage_wetted_each_m2,
                                    FUSELAGE_RADIUS_M)
    d = reference_design()
    check("cell bin is Gen 5 Kn",
          config.CELL_BIN_DEFAULT == "Kn", config.CELL_BIN_DEFAULT)
    check("cell pitch is 167 mm",
          abs(config.CELL_PITCH_M - 0.167) < 1e-12, str(config.CELL_PITCH_M))
    check("Gen 5 current overload is FLAGGED, not a hard fail",
          config.MPPT_CURRENT_OVERLOAD_OK is True)
    kn = config.CELL_BINS["Kn"]
    check("Kn Imp exceeds GVB-8 8 A",
          kn.i_mp > config.MPPT_MAX_INPUT_A,
          f"{kn.i_mp:.2f} A vs {config.MPPT_MAX_INPUT_A:.1f} A")
    from solar_uav.components.solar_array import max_cells_per_string, pack_layout
    hbay = d.solar_bays()["hstab"]
    check("H-stab packs at least one cell row",
          hbay.rows >= 1 and hbay.capacity >= 1,
          f"rows {hbay.rows} cap {hbay.capacity}")
    h_str = pack_layout(d.solar_bays(), one_per_bay=True)["by_bay"]["hstab"]
    check("H-stab is not grown to a full Voc string",
          len(h_str) == 1 and h_str[0] < max_cells_per_string(),
          str(h_str))
    check("H-stab sits on V-stab tips",
          abs(d.hstab_z - (d.vstab_height + 0.05 * d.hstab_chord)) < 1e-9,
          f"{d.hstab_z:.3f} vs fin {d.vstab_height:.3f}")
    check("π-tail shares one tail arm",
          abs(d.vstab_arm_m - d.tail_arm_m) < 1e-9,
          f"{d.vstab_arm_m:.3f} vs {d.tail_arm_m:.3f}")
    x_c4 = d.boom_x_front
    x_vte = d.vstab_te_x
    ...
    check("boom does not include the prop offset",
          d.boom_length < config.PROP_LE_OFFSET_M + d.chord_m + d.tail_arm_m,
          f"{d.boom_length:.3f} m")
...
def run_optimizer(env):
    ...
    cols = ["span_m", "chord_m", "n_packs", "n_cells", "prop", "mass_kg",
