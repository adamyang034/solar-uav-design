"""Phase 2 — Maxeon Gen 5 solar array model: electrical output, string sizing
under the Genasun boost-MPPT constraint, mass, and geometric layout on the
wing/H-stab. Gen 5 Kn Imp exceeds the GVB-8 8 A input (FLAGGED, not a
search hard-fail).
"""
...
def vmp_cell_at_temp(t_cell_c: float, bin_name: str = config.CELL_BIN_DEFAULT) -> float:
    cell = config.CELL_BINS[bin_name]
...
class SolarArray:
    ...
    def cell(self) -> config.CellBin:
        return config.CELL_BINS[self.bin_name]

    @property
    def area_m2(self) -> float:
        return self.n_cells * config.CELL_AREA_M2
...
def wing_layout(span_m: float, chord_m: float, boom_spacing_m: float,
    ...
    Wing is three pieces: inboard bay between the booms and two outboard
    ...
    chord still fits an integer number of pitches. H-stab excludes the
    elevator (split elevons live there). On a taper, aileron chord is not
    ...
    bays = [
        ...
        PanelBay("wing_outboard_R", outboard_span, chord_m,
                 config.WING_TOP_USABLE_CHORD_FRAC, n_cells_fixed=out_cap),
        PanelBay("hstab", hstab_span_m, hstab_chord_m,
                 (1.0 - elevator_chord_frac) * 0.9),  # FLAGGED: 10% LE margin
    ]
