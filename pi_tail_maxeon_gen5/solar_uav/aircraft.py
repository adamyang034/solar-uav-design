...
Ties geometry, mass, and aerodynamics together for the twin-boom OV-10-style
configuration:
  ...
  * two V-stabs (NACA 0010) carrying the H-stab (π-tail); one carbon
    boom per side, wing c/4 → V-stab TE, 1.5 in OD,
  ...
  * lateral/directional: aileron + split-elevator elevon roll rate, fin
    weathercock and yaw damping, differential-thrust yaw rate (no rudder, no
    dihedral credit),
...
def _hstab_min_chord_m(elevator_frac: float) -> float:
    """H-stab chord that packs one cell row after elevator keep-out.

    Not grown to a full Voc-legal string (user). Volume sizing can still
    make the chord larger.
    """
    return config.CELL_PITCH_M / max((1.0 - float(elevator_frac)) * 0.9, 0.35)
...
class Design:
    ...
    vstab_arm_set_m: float | None = None  # None = same as tail_arm_m
    ...
    def _planform(self) -> dict:
        ...
        L_tap = 0.0
        for _ in range(16):
            S_h_vol = config.TAIL_VOLUME_H * S * mac / max(self.tail_arm_m, 1e-6)
            if self.boom_spacing_set_m and self.boom_spacing_set_m > 0.05:
                boom = min(float(self.boom_spacing_set_m), 0.92 * b)
                min_h = _hstab_min_chord_m(self.elevator_frac)
                chord_h = max(S_h_vol / max(boom, 1e-9), min_h)
                S_h = boom * chord_h
            else:
                S_h = S_h_vol
                boom = float(np.sqrt(max(self.hstab_ar * S_h, 1e-8)))
                min_h = _hstab_min_chord_m(self.elevator_frac)
                chord_h = max(S_h / max(boom, 1e-9), min_h)
                S_h = boom * chord_h
            y_b_new = 0.5 * boom
        ...
        out = {
            ...
            "hstab_area": S_h,
            "hstab_span": boom,
            "hstab_chord": S_h / max(boom, 1e-9),
            "tail_volume_h": S_h * self.tail_arm_m / max(S * mac, 1e-9),
            "hstab_ar_actual": boom ** 2 / max(S_h, 1e-9),
            "y_taper": y_tap,
            "outboard_m": L_out,
    ...
    def vstab_arm_m(self) -> float:
        """π-tail: fin AC and H-stab AC share the tail arm."""
        return float(self.tail_arm_m)

    @property
    def vstab_area_total(self) -> float:
        """Both fins together. Sv = V_V S b / l_v; each fin is half of this."""
        s, b = self.wing_area, self.span_m
        lv = max(self.vstab_arm_m, 1e-6)
        return float(config.TAIL_VOLUME_V * s * b / lv)
    ...
    def boom_length(self) -> float:
        """One tube per side: wing quarter-chord to V-stab trailing edge."""
        return float(max(0.05, self.vstab_te_x - self.boom_x_front))

    @property
    def aft_boom_length(self) -> float:
        """Retired split-boom length. Always zero on the π-tail."""
        return 0.0
    ...
    def hstab_z(self) -> float:
        """Camber line on top of the V-stab tips (π-tail).

        Half of a 10% tail thickness above the tip so the lower surface sits
        on the fins. No T-tail downwash credit; handbook ε = 2 CL/(π AR).
        """
    ...
    def _solar_bays_at(self, aileron_y_inner_m: float) -> dict:
        return solar_array.wing_layout(
            ...
            hstab_span_m=self.boom_spacing, hstab_chord_m=self.hstab_chord,
    ...
    def _boom_parasite_f(self, v, rho):
        """D/q of both main tubes + both aft booms. v may be scalar or array."""
        f = 0.0
        for length, diam in (
            ...
            f = f + 2.0 * _cf_turbulent(re) * 1.1 * wet
        return f
    ...
    def yaw_inertia_lumped_kgm2(self) -> float:
        """Izz about wing c/4. Packs and pods sit on the booms, not the CL."""
        m = self.mass_breakdown()
        y_b = 0.5 * self.boom_spacing
        x_ref = 0.25 * self.mac_m
        x_main = 0.5 * (self.boom_x_front + self.vstab_te_x)
        ...
        i_v = m["vstabs"] * (self.vstab_arm_m ** 2 + y_b ** 2)
        i_boom = m["booms"] * ((x_main - x_ref) ** 2 + y_b ** 2)
        i_mp = (m["motors"] + m["props"]) * ((x_prop - x_ref) ** 2 + y_b ** 2)
        i_pod = (m["batteries"] + m["fixed"]) * ((0.20 * c) ** 2 + y_b ** 2)
    ...
    def control_surface_gap_area(self, y_inner: float | None = None) -> float:
        ...
        f_elev = 2.0 * CDA_control_surface_gaps(
            local_chord=self.hstab_chord,
            control_surface_span=0.5 * self.hstab_span,
            local_thickness_over_chord=0.10,
    ...
    def cn_r_control(self) -> float:
        """More-negative of handbook vs AeroBuildup (harder DT yaw rate)."""
        hb = self.cn_r_handbook()
        if getattr(self, "_aileron_y_inner_cache", None) is None:
            return hb
        from .asb_physics import aerobuildup_derivatives
    ...
    def differential_thrust_moment_n(self, t_one_max_n: float,
        ...
        """Yaw moment [N·m] from differential thrust.

        Max-effort (default): one motor at T_max, the other idle.
        Level-flight: one up / one down with T_L + T_R = drag (loiter-preserving).
        FLAG: motor spool-up is not modelled; this is the settled moment.
        """
        y = 0.5 * self.boom_spacing
        t_m = max(0.0, float(t_one_max_n))
        if not level_flight:
            return t_m * y
        v = self.min_airspeed(rho) if v_ms is None else float(v_ms)
        t_c = (self.cruise_thrust_each_n(v, rho) if t_one_cruise_n is None
               else max(0.0, float(t_one_cruise_n)))
        d = min(t_c, max(0.0, t_m - t_c))
        return 2.0 * d * y
    ...
    def yaw_rate_rad_s(self, t_one_motor_max_n: float,
                       v_ms: float | None = None,
                       rho: float = RHO_NIGHT,
                       t_one_cruise_n: float | None = None,
                       level_flight: bool = False,
                       against_weathercock: bool = False) -> float:
        """Steady yaw rate: leftover moment vs Cn_r (roll-helix analog).

        Default is still-air heading rate (β = 0). against_weathercock
        subtracts the YAW_TRIM β load so leftover after the static trim
        is what remains for rate.
        """
        v = self.min_airspeed(rho) if v_ms is None else float(v_ms)
        n_t = self.differential_thrust_moment_n(
            t_one_motor_max_n, t_one_cruise_n, v, rho,
    ...
    def yaw_rate_deg_s(self, t_one_motor_max_n: float,
                       v_ms: float | None = None,
                       rho: float = RHO_NIGHT,
                       t_one_cruise_n: float | None = None,
                       level_flight: bool = False,
    ...
    def yaw_accel_rad_s2(self, t_one_motor_max_n: float,
                         v_ms: float | None = None,
                         rho: float = RHO_NIGHT,
                         t_one_cruise_n: float | None = None,
                         level_flight: bool = False,
                         against_weathercock: bool = False) -> float:
        """Initial ṙ at r = 0. FLAG: no motor lag, so this is an upper bound."""
        v = self.min_airspeed(rho) if v_ms is None else float(v_ms)
        n_t = self.differential_thrust_moment_n(
            t_one_motor_max_n, t_one_cruise_n, v, rho,
            level_flight=level_flight)
    ...
    def yaw_correct_time_s(self, t_one_motor_max_n: float,
                           v_ms: float | None = None,
                           rho: float = RHO_NIGHT,
                           t_one_cruise_n: float | None = None,
                           level_flight: bool = False,
        ...
        FLAG: motor spool-up omitted (understates time).
        """
        v = self.min_airspeed(rho) if v_ms is None else float(v_ms)
    ...
    def differential_thrust_yaw_ok(self, t_one_motor_max_n: float,
        ...
        """Max-effort DT vs weathercock at YAW_TRIM β, plus loiter yaw rate.

        No rudder: this is the yaw-control check. Rate is still-air heading
        rate with thrust sum held at drag, leftover moment vs Cn_r.
        """
        v = self.min_airspeed(rho) if v_ms is None else float(v_ms)
        n_t = self.differential_thrust_moment_n(
            t_one_motor_max_n, t_one_cruise_n, v, rho, level_flight=False)
        n_a = self.weathercock_moment_n(v, rho)
        if n_t < n_a:
            return False
        r = self.yaw_rate_deg_s(
            t_one_motor_max_n, v, rho, t_one_cruise_n, level_flight=True)
    ...
    def to_asb(self, incidence: tuple[float, float] | None = None) -> asb.Airplane:
        ...
        # π-tail: H-stab on the fin tips, shared tail arm.
        hstab = asb.Wing(
            name="H-stab",
            symmetric=True,
            xsecs=[
        ...
        fuselages = [
            ...
            _boom_fuselage("boom_L", -y_b, x_c4, x_vte),
            _boom_fuselage("boom_R", y_b, x_c4, x_vte),
        ]
        return asb.Airplane(
            name="solar-twinboom",
