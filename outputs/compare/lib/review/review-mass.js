const GROUP_COLORS = {
  airframe: '#377fb3', battery: '#459575', solar_power: '#c38532',
  propulsion: '#a66890', avionics_controls: '#5c66b8', other: '#78838a',
};
...
export function renderMass(root, payload, ui) {
  const {heading,metric,sectionHead,icon,fmt,pct,esc,colors} = ui;
  ...
  draw = () => {
    ...
    root.innerHTML=heading('Mass breakdown','Aircraft allocation and component build-up · modeled, not measured',
      ...
      `<div class="metric-band mass-metrics">
        ...
        ${metric('MASS HEADROOM',mass(a.headroom_kg),state.unit,`${pct(a.headroom_kg/a.limit_kg,1)} of the mass limit`,a.headroom_kg>=0?'good':'bad')}
      ...
      <section>${sectionHead('Whole aircraft',`${groups.length} portions · 100% allocated`)}
        ...
        <div><table class="mass-allocation-table"><thead><tr><th>Portion</th><th class="num">Mass · ${state.unit}</th><th class="num">Aircraft share</th><th></th></tr></thead><tbody>${groups.map(g=>`<tr data-mass-group="${g.id}"><td><button class="mass-jump" data-jump="${g.id}"><span class="swatch" style="background:${GROUP_COLORS[g.id]}"></span>${esc(g.label)}</button></td><td class="num">${mass(g.mass_kg)}</td><td class="num">${pct(g.mass_kg/a.total_kg,1)}</td><td><button class="icon-button mass-jump-icon" data-jump="${g.id}" title="${esc(g.label)} detail" aria-label="${esc(g.label)} detail">${icon('arrow-down')}</button></td></tr>`).join('')}</tbody><tfoot><tr><td><strong>Total</strong></td><td class="num"><strong>${mass(a.total_kg)}</strong></td><td class="num">100.0%</td><td></td></tr></tfoot></table>
      ...
      <div class="callout">${icon('info')}<span>Solar cells and interconnects are allocated to Solar & power, not to airframe structure. Fixed budgets are distributed by function; each mass is counted once.</span></div>
      ${groups.map(g=>`<section class="section-band mass-detail" id="mass-section-${g.id}">
        <div class="mass-detail-heading"><div><h2 tabindex="-1"><span class="swatch" style="background:${GROUP_COLORS[g.id]}"></span>${esc(g.label)}</h2><p>${esc(g.note)}</p></div><div class="mass-detail-total"><strong>${mass(g.mass_kg)} <span>${state.unit}</span></strong><small>${pct(g.mass_kg/a.total_kg,1)} of aircraft</small></div></div>
