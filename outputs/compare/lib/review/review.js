...
import { definedLabel, definitionFor } from './review-definitions.js';
import { installDefinitionPopovers } from './review-tooltips.js';
...
const NAMES={pi_tail:'Single empennage',split:'Split empennage',conventional:'Conventional',conventional_rectangular:'Rectangular · no solar over ailerons',conventional_rectangular_solar:'Rectangular · solar over ailerons'};
const SHORT={pi_tail:'Single emp.',split:'Split emp.',conventional:'Conventional',conventional_rectangular:'Rect. no overlap',conventional_rectangular_solar:'Rect. solar overlap'};
...
const labelInfo=(name,override=null)=>definedLabel(name,data(),override);
const definitionPopovers=installDefinitionPopovers();
function checkLabel(check){const d=definitionFor(check.label,data());return labelInfo(check.label,{text:[d?.text,check.note,check.tolerance?`Numerical tolerance: ${check.tolerance} ${check.unit}.`:''].filter(Boolean).join(' '),formula:d?.formula});}
...
function cleanup(){definitionPopovers.close();cad?.dispose();cad=null;charts.forEach(c=>c.destroy());charts=[];}
...
function metric(name,value,unit,detail,cls=''){return `<div class="metric"><span class="metric-label">${labelInfo(name)}</span><div class="metric-value ${cls}">${value}<span class="unit">${unit}</span></div><span class="metric-sub">${detail}</span></div>`;}
function kv(rows){return rows.map(([k,v])=>`<div class="kv">${labelInfo(k)}<strong>${esc(v)}</strong></div>`).join('');}
...
function threshold(check){if(check.direction==='none')return 'Separate analysis';if(check.direction==='boolean')return 'Required';const op=check.direction==='min'?(check.strict?'>':'≥'):(check.strict?'<':'≤');return `${op} ${fmt(check.limit,check.unit==='cells'?0:check.id==='mass'||check.id==='span'?2:check.id==='recovery'?0:1)} ${esc(check.unit)}`;}
function actual(check){if(check.direction==='none')return '—';if(check.direction==='boolean')return check.actual?'Satisfied':'Not satisfied';if(check.id==='recovery'&&Math.abs(check.actual)<1e-6)return `${check.actual===0?'0':check.actual.toExponential(2)} pp`;return `${fmt(check.actual,check.unit==='cells'?0:check.id==='mass'?3:2)} ${esc(check.unit)}`;}
function headroom(check){if(!finite(check.headroom))return '—';const unit=check.unit==='%'||check.unit==='% MAC'?'pp':check.unit;if(check.id==='recovery'&&Math.abs(check.headroom)<1e-6)return check.headroom===0?'0 pp':`${check.headroom.toExponential(2)} pp`;return `${signed(check.headroom,check.unit==='cells'?0:check.id==='mass'?3:2)} ${esc(unit)}`;}
...
function overview(){
  ...
  const checks=['recovery','reserve','mass','span','minimum_chord','climb','solar_setback','solar_aileron'].map(id=>review.checks.find(c=>c.id===id)).filter(Boolean);
  const basis=review.provenance.method==='fixed-design hardware reevaluation'?'Fixed design':'Best evaluated candidate';
  root.innerHTML=heading('Design overview',`${basis}${p.hardware?.tail_boom?' · '+p.hardware.tail_boom.part_number+' boom':''} · clear-sky design day`,button(`Compare designs ${icon('arrow-up-right')}`,'go-compare'))+
    ...
    <div class="overview-primary"><section>${sectionHead('Aircraft configuration','SI geometry',`<button class="text-button" id="go-geometry">Geometry ${icon('arrow-up-right')}</button>`)}<div id="cad" class="cad-host"></div>
      <div class="geometry-strip"><span>${labelInfo('Wingspan')}<b>${fmt(d.span_m,3)} m</b></span><span>${labelInfo('Wing area')}<b>${fmt(d.wing_area_m2,3)} m²</b></span><span>${labelInfo('Solar array')}<b>${m.n_cells} cells · ${d.n_strings} MPPTs</b></span><span>${labelInfo('Propulsion')}<b>${m.n_motors} × ${esc(m.prop)}</b></span></div></section>
      ...
      <table class="compact"><thead><tr><th>Requirement</th><th class="num">Actual / limit</th><th class="num">Status</th></tr></thead><tbody>${checks.map(c=>`<tr><td>${checkLabel(c)}</td><td class="num">${actual(c)}<small>${threshold(c)}</small></td><td>${statusBadge(c.status)}</td></tr>`).join('')}</tbody></table>
...
const METRICS=[
  ...
  {name:'H-stab trim incidence',unit:'deg',get:p=>p.dims.incidence_hstab_deg,d:2},
  {name:'Horizontal tail arm',unit:'m',get:p=>p.dims.tail_arm_m,d:3},
  {name:'Wing LE to H-stab LE',unit:'m',get:p=>p.dims.hstab_le_x_m-p.dims.wing_le_x_m,d:3},
  {name:'Vertical tail arm',unit:'m',get:p=>p.dims.vstab_arm_m,d:3},
  {name:'Boom length',unit:'m',get:p=>p.dims.boom_length_m,d:3},
  {name:'Modeled tube length',unit:'m',get:p=>p.dims.boom_tube_length_m,d:3},
  ...
  {name:'Wing solar LE setback',unit:'mm',get:p=>p.review.checks.find(c=>c.id==='solar_setback').actual,d:1},
  {name:'Solar over ailerons',unit:'policy',get:p=>p.dims.cells_on_aileron?'Allowed':'Prohibited',text:true},
...
function compare(){
  ...
  root.innerHTML=heading('Compare configurations','Selected results, common design-day assumptions and model gates.')+
    `<div class="compare-controls"><div class="controls-row"><button id="select-current-battery" class="button" ${META.batteries.length===1?'hidden':''}>This battery</button><button id="select-all" class="button">All ${IDS.length}</button><span class="muted">${ids.length} selected</span></div>
...
function geometry(){
  ...
  const tube=p.hardware?.tail_boom;
  const tailFoilRows=[['Horizontal tail airfoil','Horizontal stabilizer'],['Vertical tail airfoil','Vertical stabilizer']].map(([label,surface])=>[label,p.airfoils?.find(f=>f.surface===surface)?.name??'Not recorded']);
  const tubeRows=tube?[['Boom tube',tube.part_number],['Boom linear mass',fmt(tube.linear_mass_kg_m,4)+' kg/m'],['Boom ID',fmt(tube.inner_diameter_m*1000,3)+' mm']]:[];
  root.innerHTML=heading('Geometry & mass','Read-only optimized geometry and modeled component allocation.')+
    ...
    section('Tail & control surfaces',[...tailFoilRows,['H-stab trim incidence',fmt(d.incidence_hstab_deg,2)+' deg'],['Horizontal tail span',fmt(d.hstab_span_m,3)+' m'],['Horizontal tail chord',fmt(d.hstab_chord_m,3)+' m'],['Horizontal tail arm',fmt(d.tail_arm_m,3)+' m'],['Wing LE to H-stab LE',fmt(d.hstab_le_x_m-d.wing_le_x_m,3)+' m'],['Vertical tail height',fmt(d.vstab_height_m,3)+' m'],['Vertical tail chord',fmt(d.vstab_chord_m,3)+' m'],['Vertical tail arm',fmt(d.vstab_arm_m,3)+' m'],['Aileron span / side',fmt(d.aileron_span_each_m,3)+' m'],['Yaw control',p.metrics.n_motors===1?'Rudder':'Differential thrust'],...(p.metrics.n_motors===1?[['Rudder chord · installed / required',`${fmt(d.rudder_installed_chord_m*1000,1)} / ${fmt(d.rudder_required_chord_m*1000,1)} mm`],['Rudder moment · available / required',`${fmt(d.rudder_available_moment_nm,2)} / ${fmt(d.rudder_required_moment_nm,2)} N·m`]]:[])])+
    section('Installation',[['Motors',p.metrics.n_motors+' × '+p.metrics.motor],['Propeller',p.metrics.prop],['Propeller diameter',fmt(d.prop_diameter_in,1)+' in'],...tubeRows,['Boom length',fmt(d.boom_length_m,3)+' m'],['Modeled tube length',fmt(d.boom_tube_length_m,3)+' m'],['Boom OD',fmt(p.metrics.boom_od_mm,2)+' mm'],['Fuselage length',fmt(d.fuselage_length_m,3)+' m'],['Solar cells',p.metrics.n_cells],['Wing solar setback / minimum',fmt(p.review.checks.find(c=>c.id==='solar_setback').actual,1)+' / '+fmt(p.review.inputs.WING_SOLAR_LE_SETBACK_M*1000,0)+' mm'],['Solar over ailerons',d.cells_on_aileron?'Allowed':'Prohibited'],['String plan',p.metrics.string_plan]])+`</div>
...
function energy(){const p=data(),e=p.energy,m=p.metrics;
  ...
  ${section('Solar & power conversion',[['Solar cells',m.n_cells],['Wing LE keep-out',fmt(p.review.inputs.WING_SOLAR_LE_SETBACK_M*1000,0)+' mm'],['Solar over ailerons',p.dims.cells_on_aileron?'Allowed':'Prohibited'],['Strings / MPPTs',p.dims.n_strings],['Cells per string',m.string_plan],['MPPT topology','Proposed buck/boost'],['MPPT mass',fmt(p.review.inputs.MPPT_MASS_KG*1000,0)+' g each'],['MPPT efficiency',pct(p.review.inputs.MPPT_EFFICIENCY,0)],['Max input current',fmt(p.review.inputs.MPPT_MAX_INPUT_A,1)+' A']])}
...
function sources(){const p=data(),r=p.review,v=r.provenance;
  root.innerHTML=heading('Model & sources','Result lineage, assumptions and unresolved engineering risks.')+
    ...
    <div class="section-band">${v.method==='fixed-design hardware reevaluation'?sectionHead('Retained design parameters','No geometry optimization')+kv(Object.entries(v.fixed_geometry).map(([k,value])=>[k,fmt(value,6)])):sectionHead('Search geometry bounds','Manifest snapshot')+`<div class="table-scroll"><table class="assumption-table"><thead><tr><th>Variable</th><th class="num">Lower</th><th class="num">Upper</th><th class="num">Start</th><th>State</th></tr></thead><tbody>${Object.entries(v.bounds).map(([k,b])=>`<tr><td>${labelInfo(k)}</td><td class="num">${fmt(b[0],4)}</td><td class="num">${fmt(b[1],4)}</td><td class="num">${fmt(b[2],4)}</td><td>${b[0]===b[1]?'Fixed':v.active_geometry.includes(k)?'Optimized':'Not in search vector'}</td></tr>`).join('')}</tbody></table></div>`}</div>`;
  function filter(){definitionPopovers.close();const q=document.getElementById('input-search').value.toLowerCase();const inputs=Object.entries(r.inputs).filter(([k])=>k.toLowerCase().includes(q));document.getElementById('inputs-body').innerHTML=inputs.map(([k,value])=>`<tr><td>${labelInfo(k)}</td><td class="num">${esc(typeof value==='number'?String(Number(value.toPrecision(10))):value)}</td><td class="muted">${INPUT_UNITS[k]|| (typeof value==='boolean'?'boolean':typeof value==='number'?'dimensionless':'identifier')}</td></tr>`).join('');document.getElementById('no-inputs').hidden=inputs.length>0;icons();}
  document.getElementById('input-search').oninput=filter;filter();
}

function mass(){charts=renderMass(root,data(),{heading,metric,sectionHead,icon,fmt,pct,esc,colors,labelInfo,closeDefinitions:()=>definitionPopovers.close()});}
...
function render(){
  ...
  document.querySelector('.snapshot-label').textContent=`${IDS.length} configurations`;
  document.getElementById('navigation').innerHTML=NAV.map(([id,title,ico])=>`<a href="?cfg=${current}&view=${id}" data-nav="${id}" class="${id===view?'active':''}" ${id===view?'aria-current="page"':''} title="${title}">${icon(ico)}<span class="nav-label">${title}</span>${id==='compare'?`<span class="nav-count">${IDS.length}</span>`:''}</a>`).join('');
...
document.querySelectorAll('[data-export]').forEach(b=>b.onclick=()=>{
  ...
  if(b.dataset.export==='csv'){
    ...
    rows.push(['Boom length definition','datum',...ids.map(()=> 'Wing leading edge to vertical-tail root leading edge; longitudinal x distance')],['Modeled tube length definition','datum',...ids.map(id=>{const d=CONFIGS[id].payload.dimension_definitions.boom_tube_length_m;return `Unchanged material length; x = ${d.start_x_m} to ${d.end_x_m} m`;})],['Snapshot built','UTC',...ids.map(()=>META.built_utc)],['Source CSV','path',...ids.map(id=>CONFIGS[id].payload.review.provenance.source_csv)],['Source SHA-256','hash',...ids.map(id=>CONFIGS[id].payload.review.provenance.source_sha256)]);
