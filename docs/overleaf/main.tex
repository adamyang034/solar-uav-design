...
\begin{quote}
\textit{If this airframe flies a clear-sky solstice day at our site, with our packs and our motors, does the repeating day stay above 20\% overnight and not get poorer?}
\end{quote}
...
\begin{intuition}
\textbf{Intuition.} Think of the battery as a water tank, the motors as a tap, and the solar array as a daytime hose. Closing is not ``does the hose fill the tank to 100\%?'' Closing is ``can the tank last until the hose starts working again, and is Tuesday's tank at least as full as Monday's?'' Extra noon solar that you cannot store does not help the night. Launching full is a one-night gift; the scored day is the next one.
\end{intuition}
...
\begin{itemize}
  ...
  \item Two Upgrade Energy GOLD V1 6S packs in parallel, Genasun GVB-8-Li-25.0V MPPTs (buck and boost; one per string).
\end{itemize}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{three_view.png}
  \caption{Current nearest-miss three-view from the live CAD mesh: $5.91\times 0.314\,\mathrm{m}$, $\lambda=0.60$, H-arm $1.28\,\mathrm{m}$, V-arm $1.70\,\mathrm{m}$, 76 cells ($26+26+16+8$), $8.74\,\mathrm{kg}$. H-stab in the wing plane; V-stabs further aft. Blue patches are C60 cells.}
  \label{fig:threeview}
\end{figure}

No payload. The mission is an 81-hour endurance record, designed to 89 hours (10\% margin). The optimizer launches the design day from a full pack (ground charge), then scores the \textbf{next} 24 hours: dawn must stay at or above 20\%, and that repeating day must not get poorer (start $\le$ end). A first night that only survives because you took off full is not closed.
...
Span & $\le 6\,\mathrm{m}$ \\
Packs & exactly two (no third pack) \\
...
\textbf{Maximize 24-hour energy margin} (watt-hours above the 20\% SOC floor on the repeating day), subject to periodicity, climb, MTOW, span, solar layout, and trim/pitch.

After the two-day march (day 1 from SOC $=1$, day 2 from wherever day 1 ended),

\begin{align}
  E_\mathrm{reserve}
    &= (\mathrm{SOC}_\mathrm{min} - 0.20)\,E_\mathrm{pack,total}, \\
  E_\mathrm{cycle}
    &= (\mathrm{SOC}_\mathrm{end} - \mathrm{SOC}_\mathrm{start})\,E_\mathrm{pack,total}, \\
  \mathrm{margin}
    &= E_\mathrm{reserve}.
\end{align}

$\mathrm{SOC}_\mathrm{min}$, $\mathrm{SOC}_\mathrm{start}$, and $\mathrm{SOC}_\mathrm{end}$ are all on \textbf{day 2}. $E_\mathrm{cycle}$ is a diagnostic: it must be $\ge -0.5\,\mathrm{Wh}$ to close, but it is not stacked into the score. If the pack could not supply the requested power at any step, margin is also capped at $-\,E_\mathrm{unmet}$.

\begin{intuition}
\textbf{Intuition.} $\mathrm{SOC}_\mathrm{min}$ is ``did we scrape the floor at dawn on the repeating day?'' $\mathrm{SOC}_\mathrm{end} \ge \mathrm{SOC}_\mathrm{start}$ is ``is Tuesday at least as rich as Monday?'' A design that holds 20\% on the first night from a full launch and then ratchets is not a winner. We do not require the pack to hit 98\%.
\end{intuition}

A design is \textbf{closed} only if all of these hold:

\begin{itemize}
  \item $\mathrm{SOC}_\mathrm{min} \ge 0.20$ (day 2)
  \item unmet energy $< 0.5\,\mathrm{Wh}$ (day 2)
  \item climb $\ge 1.5\,\mathrm{m/s}$
  \item $E_\mathrm{cycle} \ge -0.5\,\mathrm{Wh}$ (day 2 does not get poorer)
\end{itemize}

Margin on the current nearest-miss, scored on the repeating day, is worse than the old first-night figure: $\mathrm{SOC}_\mathrm{min} = 0.007$, a $-28\,\mathrm{Wh}$ cycle, and $80\,\mathrm{Wh}$ unmet. The 20\% floor is still what fails. Extra noon solar mostly spills; you need energy \emph{during the night}. The old 98\% refill gate is gone; this row also ratchets.
...
Solar layout & cells fit the bays; each series string stays within the GVB-8 Voc/power cap \\
...
Candidates that fail the SOC floor or the day-2 cycle are still written to the CSV so we can see nearest misses. Climb failure also appears in the reason string.
...
\begin{itemize}
  ...
  \item The GVB-8 bucks and boosts, so string Voc may sit above the 6S bus. Remaining caps: cold-cell Voc $\le 50\,\mathrm{V}$ (recommended STC Voc; 60\,V is the absolute input) and STC string power $\le 210\,\mathrm{W}$. That is \textbf{61 cells}/string on bin I (the 210\,W rating binds before 50\,V).
...
Packs & $\{2\}$ & locked, integer \\
Motor & A40-12L kv410 direct & not searched \\
ESC & fixed $0.060\,\mathrm{kg}$ lump & not searched \\
$N_\mathrm{cells}$ & from packing & integer, one MPPT-legal string per bay \\
...
\begin{enumerate}
  ...
  \item Pack cells: one MPPT-legal series string per bay. Reject if cells overlap or hang off the planform.
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{geometry_top.png}
    \caption{Planform (equal scale).}
  \end{subfigure}\hfill
  \begin{subfigure}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{geometry_side.png}
    \caption{Side schematic.}
  \end{subfigure}
  \caption{Independent tails on the current airplane. Boom runs wing $c/4$ to V-stab trailing edge. V-stab leading edge is $0.19\,\mathrm{m}$ behind the H-stab trailing edge. No $\pi$-tail; $z_H=0$.}
  \label{fig:geometry}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.78\textwidth]{mass_breakdown.png}
  \caption{Mass lumps on the current nearest-miss. Batteries $2.56\,\mathrm{kg}$ and wing structure $2.25\,\mathrm{kg}$ dominate. Booms are $0.65\,\mathrm{kg}$ at $0.174\,\mathrm{kg/m}$.}
  \label{fig:mass}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{airfoil_polars.png}
  \caption{Cached NeuralFoil \texttt{xlarge} polars at $\mathrm{Re}=1.5\times 10^5$. Production uses S4310 on the wing and NACA~0010 on both tails.}
  \label{fig:airfoils}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{drag_budget.png}
  \caption{Night drag budget at the flown speed $1.3 V_s = 11.42\,\mathrm{m/s}$. Wing profile 44\%, induced 33\%. Empennage plus pods, booms, and gaps is the remaining quarter.}
  \label{fig:drag}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{power_vs_speed.png}
  \caption{Night polar and $DV(V)$. Unconstrained aero min-power is $\sim$9.2\,m/s ($DV\approx 29\,\mathrm{W}$). The $1.3 V_s$ floor binds at $11.42\,\mathrm{m/s}$ and costs about $7\,\mathrm{W}$ of aero / $\sim$10\,W of bus.}
  \label{fig:power-speed}
\end{figure}
...
\subsection{String voltage and power (GVB-8)}

The GVB-8 input range is $0$--$60\,\mathrm{V}$ and it charges a 25\,V bus from a panel that may sit above or below that bus (buck and boost). The old rule ``string Voc must stay below the depleted 6S bus so the converter can always boost'' is therefore dropped. Remaining datasheet caps, evaluated conservatively:

\begin{equation}
  N_{\max} = \min\left(
    \left\lfloor \frac{50}{V_\mathrm{oc}(10^\circ\mathrm{C})} \right\rfloor,
    \left\lfloor \frac{210\,\mathrm{W}}{P_\mathrm{mp,STC}} \right\rfloor
  \right)
  = \min(70, 61) = 61
\end{equation}

on bin I ($V_\mathrm{oc}(10^\circ\mathrm{C})\approx 0.713\,\mathrm{V}$, $P_\mathrm{mp}=3.40\,\mathrm{W}$). The 210\,W recommended panel power binds first. A bay that wants more than 61 cells still needs a second string (extra MPPT mass) or it is illegal under one-string-per-bay.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.92\textwidth]{solstice_day.png}
  \caption{El Mirage solstice design day: Ineichen GHI and plane-of-array after IAM and encapsulation, sun elevation, and the hot static cell-temperature model (no flight-cooling credit). Peak cell $T$ is calibrated to $75^\circ\mathrm{C}$.}
  \label{fig:solstice}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.78\textwidth]{battery_ocv.png}
  \caption{FLAGGED generic Si-anode OCV curve, times 6. The 20\% floor is the same 80\% usable window as the 773\,Wh pack budget.}
  \label{fig:ocv}
\end{figure}
...
The march starts at the \textbf{last afternoon timestep} where $P_\mathrm{net} > 1\,\mathrm{W}$, with SOC $= 1$ (launch / ground charge), wraps 24 hours, then repeats that wrap \textbf{without resetting the pack}. Day 2 is the scored loop: the night is fully counted, and we test that Tuesday is not poorer than Monday.
...
Usable pack is 773\,Wh. Shoulders (dawn/dusk when the array makes some but not all of cruise) contribute on the order of 100+\,Wh that a naive $P\times t$ ignores. The honest integral is still larger than 773\,Wh --- that is why we miss. On the repeating day the nearest-miss bottoms at $\mathrm{SOC}=0.007$ with a $-28\,\mathrm{Wh}$ cycle. Closing is ``about 11\,W off the night bus'' or ``a bit more pack,'' not ``the polar must be twice as good.''
\end{examplebox}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.92\textwidth]{mission_power.png}
  \caption{Local-time power on 21 June. Array peaks near $200\,\mathrm{W}$; cruise is $75\,\mathrm{W}$. Battery-true night is the 14.0\,h where the gold curve is below the blue load, not civil sunset-to-sunrise.}
  \label{fig:mission-power}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.92\textwidth]{mission_soc.png}
  \caption{Two-day march: launch full at the last afternoon surplus, then score day 2. On the repeating day the nearest-miss bottoms at $\mathrm{SOC}=0.007$ and still ratchets. Unmet energy appears on day 2 once the first-night gift is spent.}
  \label{fig:mission-soc}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{tradespace.png}
  \caption{812 scored DE candidates, none closed. Colour on the left is night bus; on the right is margin. The nearest-miss (red) is the lightest / slowest / lowest-watt corner that still packs a legal array. The 12\,kg MTOW wall is unused.}
...
Day array $\sim$200\,W vs.\ $\sim$75\,W cruise. Climb is an order of magnitude above requirement. MTOW has $\sim$3\,kg of unused budget on the nearest-miss. Span is at the wall. The SOC floor on the repeating day is what fails. \textbf{Every other subsystem is a servant of night bus watts and night duration.}
...
\begin{itemize}
  ...
  \item Takeoff roll, landing, or a 89-hour trace with degrading packs --- only a two-day march (launch-full, then a repeating 24\,h loop).
...
Cold cells $10^\circ\mathrm{C}$ for Voc & 50\,V recommended Voc (not the old 19.2\,V bus cap) \\
GVB-8 50\,V Voc / 210\,W panel & 61 cells/string on bin I; not the old 26-cell bus cap \\
...
\code{solar\_uav/components/solar\_array.py} & C60, GVB-8 string cap, bay packing \\
...
This folder is an Overleaf project. Set the compiler to \textbf{pdfLaTeX} and the main document to \code{main.tex}. Run two passes so the table of contents resolves.
...
Figures live in \code{figures/} and were generated from the live model on 17 August 2026 by \code{scripts/export\_overleaf\_figures.py} (\code{reference\_design()} plus \code{outputs/phase4\_candidates.csv}). If \code{config.py} changes, re-run that script before trusting a quoted watt or a plot.
