...
def test_fins_and_loiter():
    ...
    check("Vv is held when V-arm grows",
          abs(aft.tail_volume_v_actual() - d0.tail_volume_v_actual()) < 1e-6,
          f"{aft.tail_volume_v_actual():.4f} vs {d0.tail_volume_v_actual():.4f}")
    check("|Cn_r| grows with V-arm at fixed Vv",
          abs(aft.cn_r()) > abs(d0.cn_r()),
          f"aft {aft.cn_r():.4f} vs {d0.cn_r():.4f}")
