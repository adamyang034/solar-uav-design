...
def test_mass_and_geometry():
    ...
    check("H-stabs do not meet (no π-tail)",
          d.hstab_span < d.boom_spacing - 1e-6,
          f"{d.hstab_span:.3f} vs boom {d.boom_spacing:.3f}")
    check("no H-stab solar bay",
          "hstab" not in d.solar_bays(),
          str(list(d.solar_bays())))
...
def test_site_motor_boom_fuse_pack():
    ...
    check("tails are not sized from Vh/Vv",
          config.USE_TAIL_VOLUME is False)
    ...
    from solar_uav.aircraft import (reference_design, fuselage_wetted_each_m2,
                                    FUSELAGE_RADIUS_M)
    d = reference_design()
    check("reference motor is 12L", d.motor_name == config.MOTOR_DEFAULT,
          d.motor_name)
    check("H-stab sized to SM_MIN",
          d.static_margin() >= config.STATIC_MARGIN_MIN - 1e-4,
          f"SM={d.static_margin():.3f} vs min {config.STATIC_MARGIN_MIN}")
    x_c4 = d.boom_x_front
    x_vte = d.vstab_te_x
    check("boom starts at wing TE",
          abs(d.boom_x_front - d.chord_m) < 1e-9,
          f"{d.boom_x_front:.3f} vs chord {d.chord_m:.3f}")
    check("boom is wing TE to V-stab TE",
          abs(d.boom_length - (x_vte - x_c4)) < 1e-9,
          f"{d.boom_length:.3f} vs {x_vte-x_c4:.3f}")
    check("split tail areal mass is 0.75 of the π-tail",
          abs(config.TAIL_AREAL_MASS / config.TAIL_AREAL_MASS_PI
              - config.TAIL_MASS_VS_PI) < 1e-12,
          f"{config.TAIL_AREAL_MASS:.3f}")
