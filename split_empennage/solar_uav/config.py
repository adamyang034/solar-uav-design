...
# ---------------------------------------------------------------------------
# Solar cells — SunPower C60 (datasheet, unlaminated, STC)
# ---------------------------------------------------------------------------
...
C60_BINS = {
    "G": CellBin("G", 3.34, 0.574, 5.83, 0.682, 6.24, 0.218),
    "H": CellBin("H", 3.38, 0.577, 5.87, 0.684, 6.26, 0.221),
    "I": CellBin("I", 3.40, 0.581, 5.90, 0.686, 6.27, 0.223),
    "J": CellBin("J", 3.42, 0.582, 5.93, 0.687, 6.28, 0.225),
}
CELL_BIN_DEFAULT = "I"            # FLAGGED: user to confirm purchased bin

CELL_GAMMA_PMP = -0.0032          # power temp coefficient, 1/degC (datasheet)
CELL_BETA_VMP = -0.0018           # voltage temp coefficient, V/degC (datasheet)
CELL_SIDE_M = 0.125               # physical cell (datasheet)
CELL_PITCH_M = 0.130              # layout footprint incl. margin (user)
CELL_MASS_KG = 0.0065             # FLAGGED: C60 ~6.5 g (Gen III datasheet value)
CELL_INTERCONNECT_MASS_KG = 0.0015  # FLAGGED: dogbones + tabbing per cell
...
TAIL_AREAL_MASS_PI = 0.44         # kg/m^2 wetted on the π-tail airframe
TAIL_AREAL_MASS = TAIL_AREAL_MASS_PI * TAIL_MASS_VS_PI
# 1.5 in OD carbon tube (user, 2026-08-16). Was 0.410 kg/m Ø24 mm.
# Same tube; independent tails let it start at the wing TE, not c/4.
BOOM_MASS_PER_M = 0.290           # kg/m
...
FIXED_MASSES_KG = {
    ...
    "vstab_hstab_interfaces": 0.060 * TAIL_MASS_VS_PI,  # no bridging spar
}

# ---------------------------------------------------------------------------
# Geometry / layout (user)
# ---------------------------------------------------------------------------
# 130 mm pitch already includes cell size + gap. A 300 mm chord still
# fits two rows. The top surface is fully packable (no extra usable-frac
# knockdown). Cells are wing-only in this fork.
WING_TOP_USABLE_CHORD_FRAC = 1.0
HSTAB_ELEVATOR_CHORD_FRAC = 0.30    # FLAGGED: elevator chord; not a solar keep-out
# Historical π-tail volume coefficients. Not used for geometry in this fork.
# Tails are inverted from SM_MIN / Cn_β / |Cn_r|. No T-tail downwash or
# endplate credit. FLAGGED: SM_MIN 0.08 is the flying floor, not 0.12 target.
TAIL_VOLUME_H = 0.50
TAIL_VOLUME_V = 0.035
USE_TAIL_VOLUME = False
# Two identical fuselages, each with its own T-tail. Not a π-tail.
# No solar on the H-stabs. Per-panel AR for lift slope / induced drag —
# do not treat the pair as one span. No T-tail downwash credit.
# FLAGGED: two extra H-stab tips vs a bridging surface; e_H = 0.70 in
# the polar is already the knockdown.
SOLAR_ON_HSTAB = False
N_HSTABS = 2
HSTAB_SPAN_BOOM_FRAC_MAX = 0.92     # each H-stab < boom spacing so they
                                    # never meet (would recreate a π-tail).
                                    # Cap span and grow chord (lower AR).
VSTAB_AREA_REF_TAIL_ARM_M = 1.30    # unused (no Vv area floor)
STATIC_MARGIN_TARGET = 0.12         # FLAGGED: CG at quarter chord assumption
STATIC_MARGIN_MIN = 0.08            # tails are sized to this, not V_H
...
VSTAB_AR = 2.5                      # FLAGGED: per V-stab; slimmer than 1.8
...
# Lateral/directional. No rudder. Yaw is not a sizing or optimizer
# constraint in this fork (user): fins are structural T-tail posts.
# Autopilot must hold Dutch roll / spiral. Differential thrust is
# available but not required to trim a 5 deg sideslip.
...
FIN_SIDEWASH_ETA = 0.90             # unused for sizing; reported only
CN_BETA_MIN = 0.10                  # unused (REQUIRE_YAW_STABILITY = False)
CN_R_ABS_MIN = 0.030                # unused
YAW_TRIM_SIDESLIP_DEG = 5.0
REQUIRE_YAW_STABILITY = False
VSTAB_CHORD_MIN_M = 0.08            # structural post: boom fitting / H-stab
# Short T-tail posts sit in the wing wake. VLM reported ~1.35× handbook ε;
# size the H-stab at this k so SM_MIN still holds. No height credit.
HSTAB_DOWNWASH_K_SIZE = 1.40
...
CONTINUOUS = {
    ...
    # User: 300–520 mm. Start 310 mm (two C60 rows). High = four cell pitches.
    "chord_m": (0.300, 4 * CELL_PITCH_M, 0.310),
    # User: start 0.90 m. Short arm grows the tail; long arm is boom mass.
...
REF_BOOM_SPACING_M = OPT_START["boom_spacing_m"]
