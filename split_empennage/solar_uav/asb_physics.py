...
def vlm_cruise(design) -> dict | None:
    ...
    try:
        ...
        y_c = 0.5 * float(design.boom_spacing)
        half = 0.5 * float(design.hstab_span)
        ys = np.linspace(y_c - 0.4 * max(half, 0.02),
                         y_c + 0.4 * max(half, 0.02), 7)
...
def downwash_k(design) -> float:
    """ε_VLM / ε_handbook at the VLM CL. <1 means the T-tail sits in weaker
    downwash — production must not take that credit."""
...
def component_inertia(design) -> dict | None:
    ...
    x_boom_c = 0.5 * (float(design.boom_x_front) + float(design.vstab_te_x))
    L_boom = float(design.boom_length)
    t_w = 0.10 * c
    ...
    parts = [
        ...
        prism(0.5 * mb["hstab"], design.hstab_chord, design.hstab_span, t_t,
              x_h, -y_b, z_h),
        prism(0.5 * mb["hstab"], design.hstab_chord, design.hstab_span, t_t,
              x_h, y_b, z_h),
        prism(0.5 * mb["vstabs"], design.vstab_chord, t_t, design.vstab_height,
              x_h, -y_b, 0.5 * design.vstab_height),
        prism(0.5 * mb["vstabs"], design.vstab_chord, t_t, design.vstab_height,
              x_h, y_b, 0.5 * design.vstab_height),
        prism(0.5 * mb["booms"], L_boom, config.BOOM_DIAMETER_M, config.BOOM_DIAMETER_M,
              x_boom_c, -y_b, 0.0),
        prism(0.5 * mb["booms"], L_boom, config.BOOM_DIAMETER_M, config.BOOM_DIAMETER_M,
              x_boom_c, y_b, 0.0),
        ...
        _place(mass_properties_of_sphere(0.5 * mb["props"], 0.15), x_prop, y_b, 0.0),
        prism(mb["batteries"] + mb["fixed"], 0.20 * c, 0.30 * b, 0.08,
              x_c4, 0.0, 0.0),
    ]
...
def audit_design(design) -> dict:
    ...
    return {
        ...
        "k_raw": k,
        "flags": _flags(k, e_hb, e_v, clp_hb, cnb_hb, cnr_hb, iyy_hb, ab, iner, xf),
    }


def _flags(k, e_hb, e_v, clp_hb, cnb_hb, cnr_hb, iyy_hb, ab, iner, xf) -> list[str]:
    flags = []
    if k < 1.0 - 1e-6:
        flags.append(
            f"VLM downwash at the T-tails is {k:.2f}× handbook; keeping handbook "
    ...
    if ab:
        ...
        if abs(ab["cnr"]) > abs(cnr_hb) + 1e-4:
            flags.append(
                ...
                f"{abs(cnr_hb):.3f}; keeping handbook (weaker damping is conservative)")
