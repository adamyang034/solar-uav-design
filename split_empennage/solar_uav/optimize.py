...
Aero variables (span, chord, tail arm, boom spacing, taper, washout,
elevator fraction) are continuous. Integer / catalog quantities stay
...
def _init_de_worker(env, motor_key, prop_names, prop_cache, n_packs,
                    one_string_per_bay):
    drive = drive_for(motor_key)
    systems = {n: PropulsionSystem(prop=prop_cache[n], motor=drive)
               for n in prop_names}
    _DE_WORKER.update(
        env=env, motor_key=motor_key, prop_names=list(prop_names),
        prop_cache=prop_cache, drive=drive, systems=systems,
        n_packs=int(n_packs), one_string_per_bay=bool(one_string_per_bay),
    )


def _eval_vector(x):
    """Score one DE vector in a worker. Returns (score, row_or_none)."""
    x = np.asarray(x, dtype=float)
    w = _DE_WORKER
    d = design_from_x(
        x, n_packs=w["n_packs"], motor_name=w["motor_key"],
        one_string_per_bay=w["one_string_per_bay"])
    row = evaluate_design(
...
def search(env: pd.DataFrame | None = None,
    ...
    Voc-legal series string (no H-stab bay in this fork). boom_grid of 0
    means REF boom spacing.
                                ...
                                for cps, cps_label in cps_iter:
                                    ...
                                    for n_str in n_str_choices:
                                      for wo_f, wo_d in washouts:
                                        ...
                                        if (config.REQUIRE_YAW_STABILITY
                                                and not d.differential_thrust_yaw_ok(
                                                    t_one, v_n)):
                                            n_skip += 1
                                            continue
                                        n_eval += 1
                                        ...
                                        rows.append({
                                            ...
                                            "hstab_span_each_m": d.hstab_span,
...
def design_from_x(x, *, n_packs: int = 2, motor_name: str | None = None,
    ...
    span, chord, lt, boom, lam, f, wo, wo_s, elev = (float(v) for v in x)
    if lam >= 0.995:
        lam, f = 1.0, 1.0
    boom = float(np.clip(boom, 0.4, 0.92 * span))
    return Design(
        span_m=span, chord_m=chord, tail_arm_m=lt,
        n_packs=int(n_packs),
        ...
        washout_tip_deg=max(0.0, wo),
        washout_start_frac=float(np.clip(wo_s, 0.0, 1.0)),
    )


def _candidate_record(d: Design, mres, motor_key: str, pname: str,
                      one_string_per_bay: bool) -> dict:
    bays = d.solar_bays()
    p_a, p_e = d.roll_rate_parts_deg_s()
    return {
        "span_m": d.span_m,
...
def evaluate_design(d: Design, env: pd.DataFrame, prop_names: list[str],
    ...
    if (config.REQUIRE_YAW_STABILITY
            and not d.differential_thrust_yaw_ok(t_one, v_n)):
        return None
    mres = simulate(d, env, psys)
...
def _rows_to_frame(rows) -> pd.DataFrame:
    ...
    keys = ["span_m", "chord_m", "tail_arm_m", "boom_spacing_m",
            "taper_ratio", "taper_start_frac", "washout_tip_deg",
            "washout_start_frac", "elevator_frac", "n_packs", "prop"]
...
def search_continuous(env: pd.DataFrame | None = None,
    ...
    def _snapshot(rows, tag: str = ""):
        ...
        if not verbose:
            return
        if df_tmp.empty:
            print(f"  {tag}: no feasible rows yet", flush=True)
            return
        best = df_tmp.iloc[0]
