...
async function main(){
  ...
  try{
    ...
    for(const id of ids){
      ...
      await page.locator('[data-expand-all="solar_power"]').click();
      assert(await page.locator('[data-node="cell_material"]').isVisible());
      assert(await page.locator('[data-node="cell_interconnects"]').isVisible());
      await page.locator('[data-mass-unit="g"]').click();
      assert.equal((await page.locator('[data-expand-all="solar_power"]').innerText()).trim(),'Collapse all');
      assert(await page.locator('[data-node="cell_material"]').isVisible());
      await page.locator('[data-mass-unit="kg"]').click();
      await page.locator('[data-expand-all="solar_power"]').click();
      assert.equal((await page.locator('[data-expand-all="solar_power"]').innerText()).trim(),'Expand all');
      assert.equal(await page.locator('[data-node="cell_material"]').count(),0);
      await page.locator('[data-jump="solar_power"]').first().click();
      await page.waitForTimeout(500);
      assert.equal(await page.evaluate(()=>document.activeElement.closest('section').id),'mass-section-solar_power');
    ...
    await page.locator('[data-battery="6s_36ah"]').click();
    await page.evaluate(()=>scrollTo(0,0));
    await page.screenshot({path:path.join(out,'mass-desktop-top.png')});
    await page.locator('#mass-section-solar_power').screenshot({path:path.join(out,'solar-detail.png')});
    ...
    for(const width of [1024,768,390]){
      ...
      await page.locator('[data-expand="solar_power/solar_cells"]').click();
      await page.locator('#mass-section-solar_power').screenshot({path:path.join(out,`detail-${width}.png`)});
      await page.locator('[data-expand="solar_power/solar_cells"]').click();
    }
