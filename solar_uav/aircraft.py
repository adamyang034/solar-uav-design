...
class Design:
    ...
    def _planform(self) -> dict:
        ...
        for _ in range(16):
            ...
            if self.boom_spacing_set_m and self.boom_spacing_set_m > 0.05:
                ...
                min_h = config.CELL_PITCH_M / max(
                    (1.0 - self.elevator_frac) * 0.9, 0.35)
    ...
    def _vstab_at_arm(self, lv: float) -> tuple[float, float, float]:
        """(Sv_total, height each, chord each) at a trial V-stab AC arm."""
        lv = max(float(lv), 1e-6)
        sv = float(config.TAIL_VOLUME_V * self.wing_area * self.span_m / lv)
        each = 0.5 * sv
        h = float(np.sqrt(max(self.vstab_ar * each, 1e-16)))
        c = each / max(h, 1e-9)
        return sv, h, c
    ...
    def aft_boom_length(self) -> float:
        """Retired split-boom length. Always zero (one tube per side)."""
        return 0.0
    ...
    def to_asb(self, incidence: tuple[float, float] | None = None) -> asb.Airplane:
        ...
        # H-stab in the wing plane; V-stabs independently further aft.
        hstab = asb.Wing(
            name="H-stab",
            symmetric=True,
            xsecs=[
