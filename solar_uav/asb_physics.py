...
def downwash_k(design) -> float:
    """ε_VLM / ε_handbook at the VLM CL. <1 means the H-stab sits in weaker
    downwash (T-tail height credit) — production must not take that."""
...
def _flags(k, e_hb, e_v, clp_hb, cnb_hb, cnr_hb, iyy_hb, izz_hb, ab, iner, xf) -> list[str]:
    ...
    if k < 1.0 - 1e-6:
        flags.append(
            f"VLM downwash at the H-stab is {k:.2f}× handbook; keeping handbook "
