...
Given a required total thrust at an airspeed, finds the RPM on the
single motor, the bus electrical power, and the maximum thrust available
...
def shortlist_props(d_min: float | None = config.PROP_DIAMETER_IN_RANGE[0],
    ...
    Twin-motor layouts needed a matching CW/CCW pair. Conventional
    (N_MOTORS=1) does not: every shortlisted map is eligible.
    """
    download_database()
