...
Ties geometry, mass, and aerodynamics together for a conventional
single-fuselage tractor (fork of the single-empennage optimizer):

  * S4310 wing, three pieces for packing: rectangular center panel + two
    ...
    fits an integer pitch. No cells on the H-stab.
  * One H-stab (NACA 0010) in the wing plane, span from V_H × AR (not from
    the center-panel width),
  * one V-stab (NACA 0010) further aft, LE behind H-stab TE; one carbon
    boom on the centerline, wing c/4 → V-stab TE,
  * one fuselage pod from wing TE to the prop disk (CAD-scaled wetted area),
    one tractor prop,
  ...
  * wing/H-stab incidence from NeuralFoil α(CL) and CM(CL) so the boom
    flies at ~0° at night cruise and the tail trims with elevator ≈ 0,
  ...
  * lateral/directional: aileron roll (no split-elevon credit), fin
    weathercock and yaw damping, rudder yaw (no differential thrust, no
    dihedral credit),
...
def reference_design(**kwargs) -> Design:
    ...
    fields = dict(
        ...
        washout_tip_deg=config.REF_WASHOUT_TIP_DEG,
        washout_start_frac=config.REF_WASHOUT_START_FRAC,
...
def _boom_fuselage(name: str, y: float, x0: float, x1: float,
                   diameter_m: float | None = None) -> asb.Fuselage:
    r = 0.5 * (BOOM_DIAMETER_M if diameter_m is None else float(diameter_m))
    x1 = max(float(x1), float(x0) + 0.01)
    return asb.Fuselage(
        name=name,
        xsecs=[
...
class Design:
    ...
    def _planform(self) -> dict:
        """Solve center-panel width ↔ area/MAC. Taper starts on the outboards.

        H-stab span is from V_H × AR, not from the center panel (the panel
        is a packing joint, not two booms).
        """
        ...
        L_out = 0.0
        L_tap = 0.0
        default_panel = min(10 * config.CELL_PITCH_M, 0.92 * b)
        for _ in range(16):
        ...
        if self.boom_spacing_set_m and self.boom_spacing_set_m > 0.05:
            ...
            boom = 2.0 * y_b
        S_h = config.TAIL_VOLUME_H * S * mac / max(self.tail_arm_m, 1e-6)
        span_h = float(np.sqrt(max(self.hstab_ar * S_h, 1e-8)))
        span_cap = float(config.HSTAB_SPAN_FRAC_MAX) * b
        if span_h > span_cap:
            span_h = span_cap
        chord_h = S_h / max(span_h, 1e-9)
        ...
        out = {
            ...
            "hstab_span": span_h,
            "hstab_chord": chord_h,
            "tail_volume_h": S_h * self.tail_arm_m / max(S * mac, 1e-9),
            "hstab_ar_actual": span_h ** 2 / max(S_h, 1e-9),
            "y_taper": y_tap,
            "outboard_m": L_out,
    ...
    def _vstab_at_arm(self, lv: float) -> tuple[float, float, float]:
        """(Sv, height, chord) of the single fin at a trial V-stab AC arm."""
        lv = max(float(lv), 1e-6)
        sv = float(config.TAIL_VOLUME_V * self.wing_area * self.span_m / lv)
        h = float(np.sqrt(max(self.vstab_ar * sv, 1e-16)))
        c = sv / max(h, 1e-9)
        return sv, h, c
    ...
    def boom_x_front(self) -> float:
        """Boom attaches at wing quarter-chord (unswept LE, root chord)."""
    ...
    def boom_length(self) -> float:
        """One centerline tube: wing quarter-chord to V-stab trailing edge."""
        return float(max(0.05, self.vstab_te_x - self.boom_x_front))

    @property
    def aft_boom_length(self) -> float:
        """Retired split-boom length. Always zero."""
        return 0.0
    ...
    def hstab_z(self) -> float:
        """H-stab camber in the wing plane. No T-tail downwash credit."""
        return 0.0
    ...
    def _mass_at_aileron_y(self, y_inner: float) -> float:
        ...
        return float(
            ...
            + config.N_BOOMS * self.boom_length * config.BOOM_MASS_PER_M
    ...
    def mass_breakdown(self) -> dict:
        m = {
            ...
            "booms": config.N_BOOMS * self.boom_length * config.BOOM_MASS_PER_M,
    ...
    # ---- aerodynamics --------------------------------------------------------
    def _boom_parasite_f(self, v, rho):
        """D/q of the single main tube. v may be scalar or array."""
        f = 0.0
        for length, diam in (
            (self.boom_length, config.BOOM_DIAMETER_M),
        ):
            if length < 1e-4:
                continue
            wet = np.pi * diam * length
            re = rho * v * length / MU_AIR
            f = f + float(config.N_BOOMS) * _cf_turbulent(re) * 1.1 * wet
        return f

    def parasite_drag_area(self, v_ms, rho) -> np.ndarray:
        """Non-lifting-surface drag area f = D/q [m^2]: pod + boom + CS gaps."""
        v = np.asarray(v_ms, dtype=float)
        L_fuse = self.fuselage_length_m
        swet = self.fuselage_wetted_each_m2
        re_pod = rho * v * L_fuse / MU_AIR
        f_pods = (float(config.N_FUSELAGES)
                  * _cf_turbulent(re_pod) * POD_FORM_FACTOR * swet)
        f_booms = self._boom_parasite_f(v, rho)
    ...
    def drag_breakdown(self, v_ms: float | None = None,
        ...
        f_booms = float((self._boom_parasite_f(v_arr, rho)
    ...
    def pitch_inertia_lumped_kgm2(self) -> float:
        ...
        lv = self.vstab_arm_m
        c = self.chord_m
        x_ref = 0.25 * self.mac_m
        x_main = 0.5 * (self.boom_x_front + self.vstab_te_x)
        return (
            m["wing"] * (0.22 * c) ** 2
            + m["hstab"] * lh ** 2
            + m["vstabs"] * lv ** 2
            + m["booms"] * (x_main - x_ref) ** 2
    ...
    def yaw_inertia_lumped_kgm2(self) -> float:
        """Izz about wing c/4. Packs, motor, and boom sit on the centerline."""
        m = self.mass_breakdown()
        x_ref = 0.25 * self.mac_m
        x_main = 0.5 * (self.boom_x_front + self.vstab_te_x)
        x_prop = self.prop_x
        c = self.chord_m
        b = self.span_m
        # 0.30 b > 1/√12 ≈ 0.289 (uniform span bar); harder yaw.
        i_wing = (m["wing"] + m["solar_cells"] + m["mppts"]) * (
            (0.22 * c) ** 2 + (0.30 * b) ** 2)
        i_h = m["hstab"] * (self.tail_arm_m ** 2 + (0.30 * self.hstab_span) ** 2)
        i_v = m["vstabs"] * (self.vstab_arm_m ** 2)
        i_boom = m["booms"] * ((x_main - x_ref) ** 2)
        i_mp = (m["motors"] + m["props"]) * ((x_prop - x_ref) ** 2)
        i_pod = (m["batteries"] + m["fixed"]) * ((0.20 * c) ** 2)
        return float(i_wing + i_h + i_v + i_boom + i_mp + i_pod)

    def yaw_inertia_kgm2(self) -> float:
        """Larger of lumped Izz vs CAD-located components (harder yaw)."""
        lumped = self.yaw_inertia_lumped_kgm2()
        from .asb_physics import mesh_inertia
        mesh = mesh_inertia(self)
        if mesh and mesh.get("ok"):
            return max(lumped, float(mesh["izz"]))
        return lumped
    ...
    def control_surface_gap_area(self, y_inner: float | None = None) -> float:
        """Parasite drag area f = D/q of aileron + elevator + rudder gaps."""
        ...
        f_elev = CDA_control_surface_gaps(
            local_chord=self.hstab_chord,
            control_surface_span=self.hstab_span,
            local_thickness_over_chord=0.10,
            control_surface_hinge_x=hinge_e,
            n_side_gaps=2)
        hinge_r = 1.0 - float(config.RUDDER_CHORD_FRAC)
        f_rud = CDA_control_surface_gaps(
            local_chord=self.vstab_chord,
            control_surface_span=self.vstab_height,
            local_thickness_over_chord=0.10,
            control_surface_hinge_x=hinge_r,
            n_side_gaps=2)
        return float(f_ail + f_elev + f_rud)
    ...
    def cl_delta_elevon(self) -> float:
        """Split-elevator roll. Conventional elevator is one piece: no credit."""
        return 0.0
    ...
    def cn_beta_handbook(self) -> float:
        """Weathercock Cn_β [1/rad]. One fin, Helmbold slope, no endplate."""
    ...
    def cn_beta_control(self) -> float:
        """Larger of handbook vs AeroBuildup (more weathercock to overpower)."""
        hb = self.cn_beta_handbook()
        if getattr(self, "_aileron_y_inner_cache", None) is None:
            return hb
        from .asb_physics import aerobuildup_derivatives
    ...
    def cn_r_control(self) -> float:
        """More-negative of handbook vs AeroBuildup (harder rudder yaw rate)."""
        hb = self.cn_r_handbook()
        if getattr(self, "_aileron_y_inner_cache", None) is None:
            return hb
        from .asb_physics import aerobuildup_derivatives
    ...
    def tail_volume_v_actual(self) -> float:
        return float(self.vstab_area_total * self.vstab_arm_m
                     / max(self.wing_area * self.span_m, 1e-9))
    ...
    def _yaw_qsb(self, v_ms: float, rho: float) -> float:
        q = 0.5 * rho * float(v_ms) ** 2
        return q * self.wing_area * self.span_m

    def cruise_thrust_each_n(self, v_ms: float,
                             rho: float = RHO_NIGHT) -> float:
        d = float(np.asarray(self.drag_n(v_ms, rho=rho)).reshape(-1)[0])
        return d / config.N_MOTORS

    def rudder_tau(self) -> float:
        return flap_tau(config.RUDDER_CHORD_FRAC)

    def cn_delta_rudder(self) -> float:
        """Cn_δr [1/rad]. Helmbold fin slope × Vv × τ."""
        ar_v = max(float(self.vstab_ar), 0.5)
        a_v = 2.0 * np.pi / (1.0 + 2.0 / ar_v)
        return float(config.FIN_SIDEWASH_ETA * a_v
                     * self.tail_volume_v_actual() * self.rudder_tau())

    def rudder_moment_n(self, v_ms: float | None = None,
                        rho: float = RHO_NIGHT,
                        dr_deg: float | None = None) -> float:
        v = self.min_airspeed(rho) if v_ms is None else float(v_ms)
        dr = np.radians(config.RUDDER_MAX_DEG if dr_deg is None else dr_deg)
        return self._yaw_qsb(v, rho) * self.cn_delta_rudder() * dr

    def differential_thrust_moment_n(self, t_one_max_n: float,
                                     t_one_cruise_n: float | None = None,
                                     v_ms: float | None = None,
                                     rho: float = RHO_NIGHT,
                                     level_flight: bool = False) -> float:
        """Centerline tractor: no DT moment. Yaw is the rudder."""
        return 0.0

    def weathercock_moment_n(self, v_ms: float | None = None,
                             rho: float = RHO_NIGHT,
                             beta_deg: float | None = None) -> float:
        v = self.min_airspeed(rho) if v_ms is None else float(v_ms)
        beta = np.radians(config.YAW_TRIM_SIDESLIP_DEG if beta_deg is None
                          else beta_deg)
        return self._yaw_qsb(v, rho) * self.cn_beta_control() * beta

    def yaw_damping_n_per_rads(self, v_ms: float | None = None,
                               rho: float = RHO_NIGHT) -> float:
        """Yaw moment per yaw rate [N·m / (rad/s)]. Negative."""
        v = self.min_airspeed(rho) if v_ms is None else float(v_ms)
        cnr = self.cn_r_control()
        return self._yaw_qsb(v, rho) * cnr * (self.span_m / (2.0 * max(v, 0.1)))

    def yaw_rate_rad_s(self, t_one_motor_max_n: float = 0.0,
                       v_ms: float | None = None,
                       rho: float = RHO_NIGHT,
                       t_one_cruise_n: float | None = None,
                       level_flight: bool = False,
                       against_weathercock: bool = False) -> float:
        """Steady yaw rate: leftover rudder moment vs Cn_r.

        t_one_* is unused (no DT). against_weathercock subtracts the
        YAW_TRIM β load so leftover after the static trim is what remains
        for rate.
        """
        v = self.min_airspeed(rho) if v_ms is None else float(v_ms)
        n_t = self.rudder_moment_n(v, rho)
        n_a = self.weathercock_moment_n(v, rho) if against_weathercock else 0.0
        n_r = self.yaw_damping_n_per_rads(v, rho)
        excess = n_t - n_a
        if excess <= 0.0 or n_r >= -1e-9:
            return 0.0
        return float(excess / (-n_r))

    def yaw_rate_deg_s(self, t_one_motor_max_n: float = 0.0,
                       v_ms: float | None = None,
                       rho: float = RHO_NIGHT,
                       t_one_cruise_n: float | None = None,
                       level_flight: bool = False,
                       against_weathercock: bool = False) -> float:
        return float(np.degrees(self.yaw_rate_rad_s(
            t_one_motor_max_n, v_ms, rho, t_one_cruise_n,
            level_flight, against_weathercock)))

    def yaw_accel_rad_s2(self, t_one_motor_max_n: float = 0.0,
                         v_ms: float | None = None,
                         rho: float = RHO_NIGHT,
                         t_one_cruise_n: float | None = None,
                         level_flight: bool = False,
                         against_weathercock: bool = False) -> float:
        """Initial ṙ at r = 0 from full rudder."""
        v = self.min_airspeed(rho) if v_ms is None else float(v_ms)
        n_t = self.rudder_moment_n(v, rho)
        n_a = self.weathercock_moment_n(v, rho) if against_weathercock else 0.0
        izz = max(self.yaw_inertia_kgm2(), 1e-3)
        return float(max(0.0, n_t - n_a) / izz)

    def yaw_correct_time_s(self, t_one_motor_max_n: float = 0.0,
                           v_ms: float | None = None,
                           rho: float = RHO_NIGHT,
                           t_one_cruise_n: float | None = None,
                           level_flight: bool = False,
                           against_weathercock: bool = False,
                           delta_deg: float | None = None) -> float:
        """Time to yaw `delta_deg` (default the 5° trim case), 1-DOF + damping.

        ψ(t) = r_ss (t − τ(1 − e^{−t/τ})), τ = Izz / |N_r|.
        FLAG: surface rate omitted (understates time).
        """
        v = self.min_airspeed(rho) if v_ms is None else float(v_ms)
        delta = np.radians(config.YAW_TRIM_SIDESLIP_DEG if delta_deg is None
                           else delta_deg)
        r_ss = self.yaw_rate_rad_s(
            t_one_motor_max_n, v, rho, t_one_cruise_n,
            level_flight, against_weathercock)
        if r_ss <= 1e-6:
            return float("inf")
        n_r = self.yaw_damping_n_per_rads(v, rho)
        tau = max(self.yaw_inertia_kgm2(), 1e-3) / max(-n_r, 1e-9)
        target = delta / r_ss
        t = target + tau
        for _ in range(24):
            e = np.exp(-min(t / tau, 50.0))
            f = t - tau * (1.0 - e) - target
            fp = 1.0 - e
            if abs(fp) < 1e-12:
                break
            t = max(0.0, t - f / fp)
    ...
    def differential_thrust_yaw_ok(self, t_one_motor_max_n: float = 0.0,
                                   v_ms: float | None = None,
                                   rho: float = RHO_NIGHT,
                                   t_one_cruise_n: float | None = None) -> bool:
        """Kept name for the search loop. Conventional yaw is the rudder."""
        return self.rudder_yaw_ok(v_ms, rho)

    def _named_propulsion(self):
        ...
        try:
            return PropulsionSystem(
                prop=load_prop(self.prop_name),
                motor=drive_for(self.motor_name))
        except Exception:
            return None
    ...
    def hstab_te_x(self) -> float:
        return self.hstab_le_x() + self.hstab_chord

    def dimension_report(self) -> dict:
        """Basic dimensions the mass/aero/energy models actually use."""
        mb = self.mass_breakdown()
        yaw_rate = yaw_level = yaw_t = float("nan")
        psys = self._named_propulsion()
        if psys is not None:
            v = self.min_power_speed()
            t_one = psys.max_thrust_n(v, rho=RHO_NIGHT) / config.N_MOTORS
            yaw_rate = self.yaw_rate_deg_s(t_one, v, level_flight=True)
            yaw_level = self.yaw_rate_deg_s(
                t_one, v, level_flight=True, against_weathercock=True)
            yaw_t = self.yaw_correct_time_s(t_one, v, level_flight=True)
        return {
            "span_m": self.span_m,
            ...
            "cn_r": self.cn_r(),
            "cn_r_control": self.cn_r_control(),
            "izz_kgm2": self.yaw_inertia_kgm2(),
    ...
    def to_asb(self, incidence: tuple[float, float] | None = None) -> asb.Airplane:
        ...
        vstabs = [asb.Wing(
            ...
            symmetric=False,
            xsecs=[
                asb.WingXSec(xyz_le=[x_v, 0.0, 0.0],
                             chord=self.vstab_chord, airfoil=af_t,
                             control_surfaces=[asb.ControlSurface(
                                 name="rudder", symmetric=False,
                                 hinge_point=hinge_r)]),
                asb.WingXSec(xyz_le=[x_v, 0.0, self.vstab_height],
                             chord=self.vstab_chord, airfoil=af_t),
            ],
        )]
        x_c4 = self.boom_x_front
        x_vte = self.vstab_te_x
        x_wing_te = self.chord_m
        fuselages = [
            _fuselage_pod("fuselage", 0.0, self.prop_x, x_wing_te),
            _boom_fuselage("boom", 0.0, x_c4, x_vte),
        ]
        return asb.Airplane(
            name="solar-conventional",
            xyz_ref=[0.25 * self.mac_m, 0.0, 0.0],
            s_ref=float(self.wing_area),
            c_ref=float(self.mac_m),
            b_ref=float(self.span_m),
            wings=[wing, hstab, *vstabs],
            fuselages=fuselages,
        )
