...
Built from the live Design so the viewer matches the conventional
winner / CAD, not a stale optimizer CSV. Mass lumps are one fuselage,
one boom, one motor, one V-stab — not the twin-boom split.
"""
...
def mass_model(design: Design) -> dict:
    """Named CAD lumps on the conventional centerline.

    Packs / motor / avionics are draggable in the viewer; structure is not.
    """
    ...
    add("V-stab", "structure",
        _box_mp(mb["vstabs"], design.vstab_chord, t_t, design.vstab_height,
                x_v, 0.0, 0.5 * design.vstab_height),
        "box", [design.vstab_chord, t_t, design.vstab_height], False)
    add("Boom", "structure",
        _box_mp(mb["booms"], L_main, config.BOOM_DIAMETER_M, config.BOOM_DIAMETER_M,
                x_main_c, 0.0, 0.0),
        "cylinder", [L_main, config.BOOM_DIAMETER_M], False)
    add("Motor", "propulsion",
        _sph_mp(mb["motors"], 0.04, x_prop, 0.0, 0.0),
        "sphere", [0.04], True)
    add("Prop", "propulsion",
        _sph_mp(mb["props"], 0.15, x_prop, 0.0, 0.0),
        "disk", [0.5 * design.prop_diameter_in * 0.0254], False)
    n_p = max(1, int(design.n_packs))
    p_each = mb["batteries"] / n_p
    if n_p == 1:
        pack_xs, pack_names = [x_c4], ["Pack"]
    else:
        pack_xs = [x_c4 + 0.12 * (i - 0.5 * (n_p - 1)) for i in range(n_p)]
        pack_names = [f"Pack {i + 1}" for i in range(n_p)]
    for name, x in zip(pack_names, pack_xs):
        add(name, "battery",
            _box_mp(p_each, 0.16, 0.08, 0.08, float(x), 0.0, 0.0),
            "box", [0.16, 0.08, 0.08], True)
    add("Avionics", "avionics",
        _box_mp(mb["fixed"], 0.16, 2 * fuse_r, 2 * fuse_r,
                design.boom_x_front - 0.25 * design.fuselage_length_m, 0.0, 0.0),
    ...
    return [
        {"id": "overview", "label": "Overview", "sections": [
            ...
            _section("This airplane", [
                ...
                ["24 h from full", ("CLOSED  +" if e.get("closed") else "OPEN  ")
        ...
        {"id": "geometry", "label": "Geometry & Mass", "sections": [
            ...
            _section("Tail & boom", [
                ...
                ["Center panel (packing)", _fmt(d.boom_spacing, ".2f", " m")],
            ]),
...
def payload(design: Design, energy: dict, drag: dict) -> dict:
    ...
    return {
        "title": (f"AircraftView  conventional  {design.span_m:.2f}×{design.chord_m:.2f} m  "
                  f"{design.n_cells} cells  {design.mass_kg:.2f} kg"),
        "name": (f"conventional  {design.span_m:.2f}×{design.chord_m:.2f} m  "
